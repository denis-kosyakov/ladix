...
func TestCLIGoldenMetrics(t *testing.T) {
	...
	// §C-5.3 ДО тела. Числа value.String (без подчёркиваний: 10000000, не 10_000_000).
	want := "" +
		"триггер 'выручка_оплачено < 10000000' сработал: выручка_оплачено = 2000000 (снимок) < 10000000 (порог) → истина\n" +
		"выручка оплаченных: 2000000\n" +
		"триггер 'число_заказов < 100' сработал: число_заказов = 3 (снимок) < 100 (порог) → истина\n" +
		"всего заказов: 3\n" +
...
func TestCLIGoldenSourceCSV(t *testing.T) {
	// always-on explain (§C-5): метрика-триггер печатает строку-explain §C-5.3 ДО тела.
	// Снимок дробный (2300000.5) — value.String сохраняет .5.
	want := "" +
