...
// KW_TYPE не начинает primary → печать(тип(5)) даёт ПАРС-ошибку «неожиданный элемент
...
// на парс-диагностику (эмпирически: «неожиданный элемент 'тип'»). Замок ОБЯЗАН
...
func TestDocsAlignmentA4TipReserved(t *testing.T) {
	...
	if !strings.Contains(errBuf.String(), "неожиданный элемент 'тип'") {
		t.Errorf("A4: stderr не содержит «неожиданный элемент 'тип'»: %q", errBuf.String())
	}
