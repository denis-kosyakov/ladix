...
//
// Достижимость (честная сноска, 010-A1/§SC-D-RESERVE; ранее 009/§docs-alignment A4):
// из 35 зарегистрированных синтаксически достижимы 34. С 010-A1 имя "тип" — КЛЮЧЕВОЕ
// СЛОВО лексера KW_TYPE (internal/lexer/keywords.go: "тип": KW_TYPE), а НЕ
// зарезервированное слово. KW_TYPE не начинает выражение (parsePrimary его отвергает),
// поэтому вызов тип(...) из исходника по-прежнему недостижим: печать(тип(5)) даёт
// ПАРС-ошибку «неожиданный токен 'тип'», а не "Целое" (раньше — лексическую
// «зарезервированное слово»; сменилась лишь КАТЕГОРИЯ диагностики, не факт блокировки).
// Builtin тип зарегистрирован и реализован (целевая семантика — имя типа как Строка),
// но остаётся dormant: активируется лишь в v2 введением синтаксиса вызова — аддитивно.
// Регистрацию/реализацию НЕ удалять.
