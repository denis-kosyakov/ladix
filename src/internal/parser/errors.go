...
const (
	...
	// Обработка ошибок 029 (SE-CATCH-NO-TRY): `словить` без предшествующего `пытаться`.
	msgCatchNoTry = "'словить' допустимо только после блока 'пытаться'"
	...
	// 016 B4a (§AU-10.A): «задача» — 4-й валидный вид после «когда» (эскалация
	// дедлайна) → текст расширен (тот же код SE-TRIGGER-KIND, счёт SE=14 цел).
	msgTriggerKind  = "метрика, событие, расписание или задача"
