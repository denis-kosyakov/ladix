...
// keywords — 37 ключевых слов Ladix → их виды токенов (SPEC §2, data-model §5.3).
...
var keywords = map[string]TokenType{
	...
	// Обработка ошибок 029 (2): `пытаться`/`словить` РАЗрезервированы из reservedWords
	// и сделаны ключевыми словами (try/catch уровня Statement). `бросить` остаётся reserved.
	"пытаться": KW_TRY, "словить": KW_CATCH,
	// Коннекторы источника 010-A1 (1): §SC-D-RESERVE — `тип` РАЗрезервировано из
	// reservedWords и сделано ключевым словом-атрибутом источника. В выражении
	// KW_TYPE НЕ начинает primary → `тип(5)` = парс-ошибка, builtin `тип` dormant.
	"тип": KW_TYPE,
}

// reservedWords — 9 зарезервированных слов (FR-013): токена НЕ дают, дают
// LexError L-11. Защита совместимости для будущих версий Ladix. (010-A1
// §SC-D-RESERVE: `тип` РАЗрезервировано — теперь ключевое слово KW_TYPE; 029:
// `пытаться`/`словить` РАЗрезервированы — теперь KW_TRY/KW_CATCH. `бросить`
// остаётся reserved — throw отложен.)
var reservedWords = map[string]bool{
	"параллельно": true, "модуль": true, "импорт": true, "экспорт": true,
	"повторить": true, "выбрать": true, "бросить": true,
	"асинхронно": true, "ждать": true,
}
