...
// не регистрируются — фантомный каскад исключён (FR-025@002:164 panic-mode / FR-026
// синхро-множество). Каскад от ведущего sync-lead ключевого слова (если/пока/
// вернуть/…) в позиции выражения на одной/смежной строке (напр. «пусть x = если»)
// устранён в DX1 (фича 012-mdx-diagnostics): parsePrimary потребляет такой токен ДО
// error() (parse_expr.go), поэтому ре-диспетча того же токена нет — ровно одна
// диагностика на сломанную конструкцию (contracts/parser-recovery.md). NB: FR-025 —
// омоним; здесь это panic-mode из specs/002-parser-ast/spec.md:164, не 007a/001.
