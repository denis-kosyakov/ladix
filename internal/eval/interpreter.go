...
// ResetRunState сбрасывает состояние интерпретатора, влияющее на пересчёт метрик
// между прогонами (решение #2, вариант A, research.md). Демон зовёт его в начале
// каждого тика ПЕРЕД evalMetrics: без сброса метрика на тике 2 вернёт тот же
// снимок, что на тике 1 (i.today фиксируется один раз на первый now(),
// i.recordCache живёт «на запуск») → булев результат не меняется → переход
// ложь→истина никогда не детектится → edge-детект (FR-006) молча мёртв.
//
// Сбрасывает РОВНО два поля времени-жизни-запуска (interpreter.go now() /
// source_loader.go loadSource): i.today=nil — следующий now() заново снимет дату
// вычисления через i.clock, а не застревает на дате старта демона. В serve эти
// часы — адаптер часов планировщика (cmd/ladix serve.go evalClockFromEngine), так
// что дата метрик при сбросе берётся ОТ ТЕХ ЖЕ часов, что у движка и планировщика
// (двойные часы FR-024), а не из независимого eval.SystemClock;
// i.recordCache — отбрасывает кеш-на-запуск (§9.6), источники перечитываются на
// следующем тике. Метод над полями значения-интерпретатора, без глобального
// состояния (Принцип V). Синхронизация при разделении с горутиной демона — забота
// демона (sync.Mutex вокруг всего прогона тика, EM-11), не этого метода.
func (i *Interpreter) ResetRunState() {
	i.today = nil
	i.recordCache = make(map[string][]value.Запись)
}

// Triggers возвращает реестр зарегистрированных триггеров в порядке объявления
// (i.triggers, заполнен Analyze Шаг 1d, §TR-4). Демон перечисляет их, чтобы
// исполнять метрика/событие/расписание-триггеры на тике (TriggerID — контентный
// ключ, минтится из условия триггера, см. buildTriggerKeys/CanonicalTriggerCondition;
// выровнен по индексу этого среза, EM-17.2.1). Это тот же реестр, что использует RunTriggers
// (trigger_run.go) — переиспользование, не дублирование. Возвращается срез как
// есть (read-only по контракту: вызывающий не мутирует).
func (i *Interpreter) Triggers() []*ast.TriggerDecl {
	return i.triggers
}
