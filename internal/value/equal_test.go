...
func TestEqual(t *testing.T) {
	tests := []struct {
		...
		// 011-A2 §MW-3/§MW-D-VALUE-EQ: равенство учитывает ВСЕ поля Период.
		{"скользящие 30дн равны", Период{Name: "последние", Amount: 30, Unit: "дн"}, Период{Name: "последние", Amount: 30, Unit: "дн"}, true},
		{"скользящие N разные", Период{Name: "последние", Amount: 30, Unit: "дн"}, Период{Name: "последние", Amount: 7, Unit: "дн"}, false},
		{"скользящие единицы разные", Период{Name: "последние", Amount: 1, Unit: "нед"}, Период{Name: "последние", Amount: 1, Unit: "мес"}, false},
		{"календарные с нулевыми полями равны (обр. совместимость)", Период{Name: "ежемесячно"}, Период{Name: "ежемесячно"}, true},
		{"завершённый Offset отличает", Период{Name: "ежемесячно", Offset: -1}, Период{Name: "ежемесячно"}, false},
		// Длительность (D-17): Equal по паре единица+значение БЕЗ нормализации.
		{"2дн == 2дн", Длительность{Amount: 2, Unit: "дн"}, Длительность{Amount: 2, Unit: "дн"}, true},
		{"1час == 60мин ложь (без нормализации)", Длительность{Amount: 1, Unit: "час"}, Длительность{Amount: 60, Unit: "мин"}, false},
		{"2дн != 3дн", Длительность{Amount: 2, Unit: "дн"}, Длительность{Amount: 3, Unit: "дн"}, false},
		{"длительность != целое", Длительность{Amount: 2, Unit: "дн"}, Целое{V: 2}, false},
	}
...
func TestCompare(t *testing.T) {
	tests := []struct {
		...
		// Длительность (D-17): порядок только одной единицы — по значению; разные
		// единицы → ok == false (вызывающий eval даёт TY-BINOP).
		{"2дн < 5дн", Длительность{Amount: 2, Unit: "дн"}, Длительность{Amount: 5, Unit: "дн"}, -1, true},
		{"5дн > 2дн", Длительность{Amount: 5, Unit: "дн"}, Длительность{Amount: 2, Unit: "дн"}, 1, true},
		{"2дн == 2дн порядок", Длительность{Amount: 2, Unit: "дн"}, Длительность{Amount: 2, Unit: "дн"}, 0, true},
		{"2дн ? 5мин разные единицы", Длительность{Amount: 2, Unit: "дн"}, Длительность{Amount: 5, Unit: "мин"}, 0, false},
		{"длительность↔целое несравнимы", Длительность{Amount: 2, Unit: "дн"}, Целое{V: 2}, 0, false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
