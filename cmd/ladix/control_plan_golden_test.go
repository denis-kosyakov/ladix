...
// под FixedClock{2026,6,15}, абсолютный путь примера (фича 026: data/orders.csv
// резолвится от каталога примера examples/, os.Chdir не нужен). Пинит 3 фасета
// §2-цепочки ДВУМЯ путями:
...
func TestCLIControlPlanScalarFixedClock(t *testing.T) {
	example := absExample(t, "контроль_плана.ladix")
	var out, errBuf bytes.Buffer
	// Фича 026: путь источника data/orders.csv резолвится от каталога примера (examples/),
...
func TestCLIControlPlanRunFixedClock(t *testing.T) {
	example := absExample(t, "контроль_плана.ladix")
	var out, errBuf bytes.Buffer
	// Фича 026: data/orders.csv резолвится от каталога примера (examples/) — абсолютный
	// путь примера достаточен.
