...
// t-000001 (шаг связаться_с_клиентом) → complete --data '{"итог":"перезвонит"}' →
...
func TestQuickstartSmoke_EscalationLifecycle(t *testing.T) {
	...
	// 2) complete --data: завершение задачи будит инстанс, исполняются авто-шаги
	//    догона → durable-захват итога → реальный эффект в crm → процесс выполнен.
	got = runFromRoot(t, binPath, root, "complete", procFile, "t-000001", "--data", `{"итог":"перезвонит"}`, "--db", dbPath)
	want = "задача t-000001 завершена\n" +
