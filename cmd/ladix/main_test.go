...
// T018 (016 B4a, §AU-6) был здесь — TestCLIGoldenDeadlineEscalation: дата-наивный
// golden контроль_плана.ladix через bare `run`. УДАЛЁН фичей 023-v2-finalization (W2/§F-1):
// после впайки CSV-источника `заказы` пример стал дата-зависимым (метрика-триггер
// fire-if-true считает окно «последние 30дн» по системным часам) и нерезолвимым из cwd
// теста (data/orders.csv лежит в корне репо). Замена — детерминированные FixedClock-golden
// из repo-root: TestCLIControlPlanScalarFixedClock (скаляр) + TestCLIControlPlanRunFixedClock
// (explain §C-5.3 + метрика-driven старт) в control_plan_golden_test.go.
