...
`run`, триггерные 6 методов `Store` + `ListInstancesByStatus` (отклонение по FR-022, «+6→+7») + `ErrTriggerStateNotFound` + таблицы `trigger_state`/`events`,
фронтенд `когда` (парсер/AST/семантика, предопределённые `значение`/`событие`). `repl` — вне 006
(дополнительная возможность, фича не назначена). Рестарт-скан залипших инстансов — 007 (D-4). Функциональный чеклист от 006
...
| **D-9** | Фаза атрибутов шага. | **До тела** (правка EM-9 под SPEC §11.3): (1) вычислить `исполнитель`/`срок` в кадре шага, (2) исполнить тело, (3) развилка Task/продвижение. Ошибка вычисления атрибута = runtime-ошибка шага → `провален` (как **непойманная** ошибка тела, D-14; ошибка атрибута вне тела ловушкой `пытаться`/`словить` не покрывается). |
...
| **D-14** | Провал инстанса. | **Непойманная** runtime-ошибка тела шага (или ошибка атрибута, D-9): инстанс персистируется `провален`, далее ошибка всплывает как обычная ошибка программы — канон SPEC §13 в stderr, fail-fast, **exit 1** (`run` и `complete` одинаково). **Перехват (трек #2 v3):** ошибку тела можно поймать оператором `пытаться:`/`словить:` внутри тела шага — пойманный сбой исполняет `словить`-блок, шаг завершается штатно и инстанс продвигается (развилка D-9 п.3), `Engine.fail` НЕ вызывается; эффекты и переменные, записанные до точки сбоя, не откатываются (no-rollback; execution-model EM-12). Провал наступает лишь при непойманной ошибке (нет объемлющего `пытаться`) либо ошибке внутри самого `словить`. Сбой доставки `вызвать`/`уведомить` оставляет в outbox **durable-вердикт** (запись с `Delivered=false`; `error_text` — пояснение; `docs/reliability-model.md §C-2b`): на реплее `outboxPrecheck` пере-бросает замороженную ошибку без повторной доставки (детерминированный exactly-once); если этот сбой пойман `словить`, вердикт остаётся следом в леджере, а инстанс продолжает. Двойная природа: в Store — статус, в CLI — диагностика; дополнительной CLI-строки о провале НЕТ. В 006 под дефолт-стабом `printCaller` `вызвать` всегда успешен — путь «сбой вызвать → провален» описан, но был **недостижим**; **M2 (B2)** активировал HTTP-драйвер `webhookCaller`, и сбой доставки делает этот путь реальным (обёртка `runtimeErrWrap` с `Unwrap`, §EN-8.A). Категория `ОшибкаПроцесса` в 006 **не вводится** (ей нечего нести; SPEC §13.1 строка «Процесса» остаётся зарезервированной прозой). |
| **D-15** | Process-builtins. | Реализуются в eval через тот же `ProcessRuntime` (методы «статус по id / переменные по id / открытые задачи по исполнителю»), engine транслирует к Store. `процесс '<id>' не найден` — ОшибкаВыполнения с позицией вызова (EM-13/EM-16). `задачи_пользователя("")` → все открытые (= `ListPendingTasks("")`); порядок — **по возрастанию id** (детерминизм). Реестр на момент 006: 28 активных + 7 deferred (после 008 — **35 активных / 0 deferred**, `deferredNames` пуст, `builtins.go:51`); перелочить `TestBuiltinDeferredAll` и стейл-комментарии (§EN-0). |
...
advance(inst):                                   # крутит шаги до ожидания/терминала
    ...
    loop:
        ...
        если assignee задан:                     # человеческий шаг → заснуть
            # ИДЕМПОТЕНТНОЕ создание задачи (фикс A фичи 024): перед минтом нового ID
            # проверить уже-открытую задачу на текущей паре (inst.ID, шаг.Name):
            открытая ← first(t' ∈ st.ListTasksByInstance(inst.ID) | t'.Status==открыта И t'.StepName==шаг.Name)
            если открытая ≠ ∅:                    # переиспользовать: НЕ минтить ID, НЕ SaveTask, НЕ печатать строки 3/4
                inst.статус = ожидает
                ▼ SaveInstance(inst); return     # засыпание на той же задаче
...
**Идемпотентность создания задачи (фикс A фичи 024).** Создание задачи на человеческом
шаге — query-guard, а не безусловный минт: фаза (3) сначала ищет уже-открытую задачу на паре
`(inst.ID, шаг.Name)` через `ListTasksByInstance` (фильтр `Status==открыта И StepName`) и при наличии
**переиспользует** её — нового ID не минтит, повторного `SaveTask` нет, строк создания (§EN-7, 3/4) не
печатает, лишь переводит инстанс в `ожидает`. Это закрывает дубль-задачу при **рестарте `serve`**:
рестарт-скан реактивирует залипшие `выполняется`-инстансы (automation-model §AU-6.3 `RunRestartScan`),
и без гарда повторный заход в `advance` минтил бы ВТОРУЮ открытую задачу (`SaveTask` дедупит только по
`id`, а `id` новый; схемного `UNIQUE(instance_id, step_name)` нет — гард query-уровневый, не схемный).
Согласовано с M3 exactly-once (reliability-model §C-2b.3): outbox покрывает эффекты тела шага
(`вызвать`/`уведомить`), а создание задачи теперь идемпотентно отдельным query-guard'ом — логически
дополняет таблицу «Источник эффекта → идемпотентность» из reliability-model §C-2b.3 (создание задачи
человеческого шага → query-guard `ListTasksByInstance`).
...
type ProcessRuntime interface {
    ...
    // CallExternalResult — выражение-форма «вызвать» (M2/B1, §AU-3): тот же внешний
    // эффект + захват результата как value.Value (`присвоить r = вызвать crm(x)`).
    // Под дефолт-стабом печатает ту же строку, что CallExternal, и возвращает
    // (value.None, nil); под HTTP-драйвером — декодированный ответ вебхука (§AU-4.3).
    CallExternalResult(target string, args []value.Value) (value.Value, error)

    // Notify — «уведомить» (D-13): под дефолт-драйвером одна строка в stdout
    // (best-effort, nil); под HTTP-драйвером — POST, сбой доставки → не-nil ошибка.
...
**Драйвер внешних эффектов `ExternalCaller` (M2/B2, §AU-4.1).** `CallExternal`/
`CallExternalResult`/`Notify` движок **не исполняет сам** — он делегирует инстанс-полю
`e.caller` типа `engine.ExternalCaller` (`engine/caller.go:18`: `Call(target, args) →
(value.Value, error)` + `Notify(target, args) error`). Поле **не часть шва eval** — eval о
нём не знает (граница D-1 цела), это деталь движка. Две реализации: дефолтная печать-стаб
...
*исторический снимок границы 006; активированы в 008*, реестр 35 активных / 0 deferred,
см. `docs/datetime-builtins-model.md`);
...
*после 008 deferred-граница снята — **35 активных / 0 deferred**, `deferredNames` пуст, `builtins.go:51`*).
...
без изменений (runtime-ошибки тела шага — это они и есть). Ниже — **только новый слой 006**. (Синтаксический код `SE-CATCH-NO-TRY` — висячий `словить` без `пытаться`, фронтенд трека #2 v3 — относится к парсер-каталогу `SE-*`, канон — `SPEC §13.4` / `docs/diagnostics-model.md §MDX-3`, а не к этому рантайм/CLI-реестру.)
...
Категория «Процесса» (SPEC §13.1) в 006 не порождается.

**Сбой реального драйвера `вызвать`/`уведомить` (M2/B2).** Под дефолт-стабом `printCaller`
ошибки нет — путь «сбой `вызвать` → провален» в 006 был **недостижим** (D-14). M2 активировал
`webhookCaller`: сетевой/HTTP-сбой доставки (или сбой декода ответа) → не-nil ошибка драйвера
→ eval оборачивает её в `ОшибкаВыполнения` с сохранением причины через `Unwrap` (`runtimeErrWrap`,
`eval/interpreter.go:189`; TODO D-14 закрыт фактом) — позиция узла `вызвать`/`уведомить`, шаг
провален (D-14) — **если сбой не перехвачен** объемлющим `пытаться`/`словить` (трек #2 v3); иначе исполняется `словить`-блок и инстанс продолжает. Канон §13 в stderr, exit 1. То есть путь провала инстанса от внешнего вызова
теперь реальный, а не только контракт на будущее.
...
| `вчера()` | run | ~~`функция 'вчера' не поддерживается в этой версии` — deferred-граница 7 имён НЕ сдвинулась~~ — **исторический снимок границы 006; активирована в 008** (`вчера()` теперь возвращает `Дата` = сегодня−1, реестр 35 активных / 0 deferred; см. `docs/datetime-builtins-model.md`) |
...
- **Батчинг персиста, отзыв осиротевших задач при `провален`, реестр ролей/прав** → v2
  (EM-12/EM-15, SPEC §12). *(На момент 006 сюда же относились транзакционный outbox и
  exactly-once эффектов — **реализованы в v2/020**: `Store.LoadOutbox`/`SaveOutbox`
...
+ `ListInstancesByStatus` (отклонение по FR-022, «+6→+7») и 2 таблицы, не меняя существующие 8 методов и DDL. `ProcessRuntime` (§EN-4) аддитивен:
...
при том же фронтенде. **(M2-подтверждение аддитивности:** `ProcessRuntime` дорос до 8 методов
(`CallExternalResult` для выражения-формы `вызвать`, §EN-4), `Store` — до 16 (`ListTasksByInstance`
для `inspect`: открытые И завершённые задачи инстанса, порядок ID ASC, `store/store.go:39`),
драйвер `ExternalCaller` добавлен полем движка вне шва eval — всё без поломки существующих
методов/DDL. Полная M2-поверхность — automation-model §AU.)
