...
`journal_mode = WAL`, `busy_timeout = 5000`, `foreign_keys = ON` (EM-7).
...
**Сбои Store вне хука «присвоить»/запуска.** Engine оборачивает ошибки Store через
`fmt.Errorf("<операция>: %w", err)`. Пути, инициированные Ladix-узлом (`запустить процесс`,
