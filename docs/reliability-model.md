...
1. **payload эфемерен.** `--data '{"итог":"перезвонит"}'` парсится в `value.Запись` и биндится
...
Нормальный прогон (`complete … --data '{"итог":"перезвонит"}'`):
...
1. Усиленный §2 проходит зелёным end-to-end под `serve` (CSV-источник → метрика → триггер → процесс →
   `complete --data` → авто-шаги → реальный POST `crm`).
...
| ~~**Стабильные ключи триггеров**~~ — **РЕАЛИЗОВАНО (фича 027)** | был: ключ `trg-<N>` позиционный → соседний триггер сдвигает индекс → durable `trigger_state` наследует чужой `LastBool` (латентный баг сохранности, **не косметика**). Стало: durable-ключ контентный — хеш канонизированного условия (`buildTriggerKeys`/`ast.CanonicalTriggerCondition`, FNV-1a-64 + ordinal-дизамбигуация дубликатов), позиционный `triggerID(idx)` удалён; перестановка/вставка/удаление любого (в т.ч. событие-) триггера больше не сдвигает edge-baseline соседей. Переход = миграция схемы 2→3 (`DELETE FROM trigger_state`, сброс позиционного состояния) + поведенчески-нейтральный первый тик после апгрейда (`schedule_at` праймит без срабатывания на уже прошедшем времени, FR-010). NB: это **отдельно** от C1 хеш-версионирования **тела** определения (строка выше — всё ещё открыта). |
