...
| **B3** | Payload задачи: `ladix complete <файл> <id> --данные '{…}'` | CLI + engine + eval-инжект | — |
...
| **D-AU-2** | **B2 канал = HTTP-вебхук** (не файл, против рекомендации kickoff). Драйвер инжектится Option'ом движка. **Дефолт-драйвер = печать-стаб** → держит §EN-7 golden (≥6 пинов). Реальный = `POST` JSON `{"цель": target, "данные": [args]}`. Базовый URL — CLI-флаг `--вебхук` / env `LADIX_WEBHOOK`; `.ladix` остаётся чистым (цель = логическое имя, не URL). Детерминизм тестов — `httptest`. | Нужен детерминированный приёмник без сети. Логическое имя цели — путь отступления к реестру интеграций. |
| **D-AU-3** | **B3 = payload `данные`**: read-only эфемерная `value.Запись`, декодируется единым JSON-кодеком, поднятым из `daemon/events.go` (`payloadToRecord`) в общий пакет. Без `--данные` → пустая `Запись`. | «Эфемерный» = живёт только сквозь догон одного `complete`, не персистится в инстанс (путь отступления — персист в M3). |
...
| **D-AU-9** | **Миграция = СБРОС схемы допустим** (v2 до релиза, БД тестовые). | Де-рискует `Task.Escalated` и колонку. Полные миграции → M3-C2. |
...
(`memory.go:245`) копирует `Escalated` тривиально (`cp := *t`, bool-значение). **Миграция = сброс
схемы** (D-AU-9): пересоздать БД, не ALTER. Подтверждено вживую: `SaveTask` в обоих Store —
...
(§AU-3.3, ЕДИНАЯ категория — без «Процесса»). Тайм-аут клиента — конечный (напр. 10с), путь отступления — конфиг в M3.
...
`run`, `serve`, `complete`, `start` принимают `--вебхук <URL>` (или env `LADIX_WEBHOOK`). Задан →
`NewEngine(..., WithExternalCaller(webhookCaller{baseURL, httpClient}))`. Не задан → дефолт-стаб.
**КРИТИЧНО для DoD:** под `serve` `--вебхук` проводится в ТОТ ЖЕ экземпляр движка, чьи `Notify`/`Call`
...
`completeMain` (`main.go:281`) получает опциональный флаг `--данные '<json-объект>'`. Грамматика
вызова: `ladix complete <файл> <id> [--db путь] [--данные '{…}'] [--вебхук URL] [--max-depth N]`.
Без `--данные` → пустой payload. Невалидный JSON → `ladix: неверный JSON в --данные: …` exit 2.
...
`engine` (декод ответа вебхука B2 + энкодер тела), `cmd/ladix` (декод `--данные` B3 на CLI, §AU-5.3 —
...
**Где декодируется (закрыто):** `--данные '{…}'` → `value.Запись` декодирует **CLI** (`completeTask`,
`main.go`) через `jsonval.payloadToRecord` ПЕРЕД `eng.Complete` (тут же валидация: невалидный JSON →
`ladix: неверный JSON в --данные: …` exit 2, §AU-5.1). Движок получает уже готовую `value.Запись`
...
следующий `complete` без `--данные` видит пустую `Запись` (`данные.итог` → Пусто, открытая-запись семантика).
...
- **Тело = трактовка как у РАСПИСАНИЕ-триггера (D-AU-6, lenient-scope БЕСПЛАТНО):** НИКАКОГО нового
  флага `inDeadlineTrigger` НЕ нужно. Семпроход и так НЕ проверяет объявленность плоских Ident
  («плоский Ident НЕ резолвится — declaredness рантайму», `analyze.go:407`, `:615-616`) → свободный
  `факт` в теле эскалации статической «не объявлено» НЕ даёт, резолвится в рантайме против
  инжектированных переменных инстанса (§AU-6.2.3); неразрешённый → рантайм-`ОшибкаВыполнения`. Тело
  валидируется вызовом `checkTriggerBody(body, inMetricTrigger=false, inEventTrigger=false)` — ТОЧНО
  как расписание-триггер (`analyze.go:345`, оба флага false). Следствие: `значение`/`событие` в теле
  эскалации запрещены наследуемым контекст-гардом §TR-7.A (TR-VAL-CTX/TR-EVT-CTX). Read-only барьер
  тела — рантайм (TR-BODY-RO, `NewTriggerBodyEnv`+`markBoundary`, §AU-6.2.3), отдельной аналитики не требует.
- Действия-шага (`исполнитель:`/`срок:`) в теле триггера запрещены (наследуется §TR-7.C). Разрешено
  императивное ядро + `уведомить`/`вызвать`/`запустить процесс` (реальные в B2).
...
`--db путь` (D-AU-10), `--вебхук URL`, `--max-depth N`. Конструирует Store (SQLite при `--db`),
...
|---|---|---|---|
| `run` | `dbPath:=""` → **MemoryStore** (`main.go:191`) | `--вебхук`, `--max-depth` | §EN-6 |
| `serve` | `dbPath:=""` → **MemoryStore** (`serve.go:152`) | `--вебхук`, `--interval`, `--max-depth` | демон, 4 фазы |
| `complete` | `defaultDBPath="ladix.db"` → **SQLite** (`main.go:283,348`) | `--данные` (B3), `--вебхук` | НЕ Memory |
| `tasks` | `"ladix.db"` → **SQLite** (`main.go:414,451`) | — | НЕ Memory |
| `emit` | `"ladix.db"` → **SQLite** (`emit.go:19,59`) | — | НЕ Memory |
| **`start`** (B5) | **= как complete-семья: дефолт `ladix.db` → SQLite** | `--вебхук`, `--max-depth` | новый; см. контракт ↓ |
...
(дефолт `defaultDBPath="ladix.db"`, `main.go:64`), а НЕ семье `run`/`serve` (пусто→Memory). Причина: демо-инвариант
требует, чтобы `start`→`serve`→`complete`→`inspect` БЕЗ явного `--db` всё равно указывали на ОДИН файл `ladix.db`.
Если бы `start` дефолтил в Memory (как `run`), пользователь без `--db` получил бы рассинхрон с `complete`/`inspect`.

**Предусловие демо:** `start` создаёт инстанс в БД; `serve` (та же БД) эскалирует; `complete` (та же БД)
закрывает; `inspect` (та же БД) показывает. Конструкция Store сейчас инлайн-ДУБЛИРУЕТСЯ в 5 местах
(`main.go:182-193`/`serve.go:143-154`/`main.go:348-353`/`main.go:451-456`/`emit.go:59-64`) — выделить
...
- `ladix: неверный JSON в --данные: <деталь>` (B3).
...
- `ladix: неверный URL вебхука '<URL>'` (B2 `--вебхук`).
...
- **`README:84-89`** «Что отложено (v2): `ladix start`, передача payload через `complete`» — снять оба
  ...
  интеграциями (с пометкой «по умолчанию печать-стаб; HTTP — `--вебхук`»).
...
- 4.8 `complete … --данные '{…}'` (передача результата, B3).
...
человеческая задача со сроком → просрочка → эскалация → `complete … --данные` → реальный эффект
(`--вебхук` через `httptest`, B2) → `inspect` показывает историю. Переживает рестарт (§AU-12.B). Это
...
| B2 | дефолт = §EN-7 golden байт-точно; `--вебхук` → POST `{"цель","данные"}` принят httptest |
| B3 | `--данные '{"итог":"перезвонит"}'` → след. шаг читает `данные.итог == "перезвонит"`; без флага → Пусто |
...
| Миграции схемы Store (ALTER) | D-AU-9 сброс; M3-C2 даст реальные миграции |
