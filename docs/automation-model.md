...
2. `expectLexeme("просрочена")` — **НОВЫЙ хелпер** (в `parser` его нет: есть `expect(TokenType, string)`
   ...
   иначе `p.error(pos, msgExpected(want, tok))` → код `SE-EXPECTED` (счёт SE-* этим не меняется, §AU-10.A);
...
> `parser/inventory_test.go:38` wantCodes=15; `lexer/lexerrors_test.go:29` множество L-1..L-11=11)
...
- **SE-TRIGGER-KIND — текст РАСШИРЯЕТСЯ**: `msgTriggerKind` (`parser/errors.go:29`) меняется с
  ...
  валидное продолжение). Тот же код SE-TRIGGER-KIND (счёт SE-* этим не меняется), но это РАСШИРЕНИЕ golden-строки
...
- **Малформенный эскалация-триггер** (`когда задача X…`, `когда задача просрочена Y…`, пропуск `.`/шага)
  ...
  → `ожидалось '…', получено '<лексема>'`. **Новых SE-кодов НЕТ**, счёт SE-* этим не меняется.
