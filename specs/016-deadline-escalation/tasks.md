...
> **Фикс B4a (fix(016a), §AU-6.1.3):** семпроход тела триггера ранее отвергал `уведомить`/`вызвать`
> (`case *ast.AssignAction, *ast.CallAction, *ast.NotifyAction` → `if !inStep`), делая durable-golden
> §AU-12.B (`уведомить руководитель(факт)`) недостижимым. Введён контекст `inTriggerBody`
> (`checkStmts`/`checkStmt`/`checkElse`, `analyze.go`): в теле ЛЮБОГО триггера (метрика/событие/
> расписание/дедлайн — гард общий) `вызвать` (`CallAction`)/`уведомить` (`NotifyAction`) РАЗРЕШЕНЫ
> (allow-список §AU-6.1.3), `присвоить` (`AssignAction`) — отвергнут (не «ядро» + read-only тела
> D-AU-6), действия-шага `исполнитель:`/`срок:` структурно невозможны вне шага. T013/T016 + канон
> `контроль_плана.ladix` возвращены к `уведомить руководитель(факт)`. Замки: allow-замок (уведомить/
> вызвать в теле дедлайн-триггера ПРОХОДИТ; инверсия — вернуть `if !inStep` → RED), страж
> над-ослабления (`присвоить` в теле триггера отвергнут; инверсия — добавить `!inTriggerBody` → RED),
> регресс (метрика/событие/расписание-тела с уведомить/вызвать теперь проходят — старые негативы
> сняты, позитивы в `TestAnalyzeTriggerOK`). eval=28 не сдвинут (текст переиспользован).
