...
- [X] T030 [US3] Написать 6 негативов §EN-9 в `src/cmd/ladix/main_test.go` (после шага 5 сценария Б):
...
- [X] T031 [US3] Расширить `src/internal/engine/complete_test.go` двумя фабрикованными кейсами гардов
...
- [X] T032 [P] [US3] Написать exact-match-тесты диагностик §EN-8.A (9 текстов, канон §13, позиция,
...
- [X] T033 [P] [US3] Создать testdata-фикстуры (FR-042): `src/cmd/ladix/testdata/онбординг-дрейф.ladix`
...
- [X] T034 [US3] Реализовать полный порядок гардов `Complete` строго §EN-3 в
  ...
  зависит от T025).
- [X] T035 [US3] Замаппить гарды `Complete` на CLI-тексты §EN-8.B в `src/cmd/ladix/main.go`: трансляция
...
- [X] T036 [US3] Реализовать маршрутизацию сбоев Store по инициатору (FR-018): пути от Ladix-узла
...
- [X] T037 [P] [US4] Написать `src/internal/value/equal_test.go` (или расширить существующий) — case
  ...
  FR-025).
- [X] T038 [P] [US4] Написать точечные приёмки слоя §EN-9 в `src/internal/eval/*_test.go`: печать `2дн`
...
- [X] T039 [US4] Адаптировать замки-тесты под новое поведение (должны падать до имплементации):
...
- [X] T040 [P] [US4] Активировать case `Длительность` в `src/internal/value/equal.go`: `Equal`
...
- [X] T041 [US4] Активировать рантайм `DurationLit` в `src/internal/eval/expr.go:50-51`: заменить
...
- [X] T042 [US4] Активировать 3 процессные встроенные в `src/internal/eval/builtins.go`: убрать
...
- [X] T043 [US4] Удалить мёртвую рантайм-заглушку: убрать `deferredConstruct`
...
- [X] T044 [US4] Перевернуть замки-комментарии инвентаря §EN-0 (FR-028): счёт `interpreter.go:20`
...
- [X] T045 [P] Синхронизировать `examples/MANIFEST.md` (строка онбординга): код 0, golden сценария А,
  id детерминированы, маска только `<время>` (FR-042, SC-008).
- [X] T046 Прогнать финальные гейты SC-007 из `src/`: `go build ./...`, `go vet ./...`, `gofmt -l .`
...
- [X] T047 Проверить инвариант D-2 (SC-009): `grep -rn "time.Now()" internal --include='*.go' | grep -v
  '_test.go'` → ровно 2 места (`internal/eval/clock.go` дневные, `internal/engine/clock.go` lifecycle);
  любое третье вхождение — нарушение.
- [X] T048 Подтвердить регрессы: golden-вывод 004 (`TestGoldenSM10`, §SM-10) не изменился ни на байт;
  все тесты 001–005 зелёные (FR-043, SC-007).
- [X] T049 [P] Тест-реестр всех 11 stdout-форматов §EN-7 exact-match: явная карта формат→покрывающий
