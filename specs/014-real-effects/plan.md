...
4. **Кодек `internal/jsonval`** (НОВЫЙ нейтральный пакет) — лифт декодера `decodeValue`/`payloadToRecord` из `daemon/events.go` (без зависимости на `*Daemon`) + НОВЫЙ энкодер `value → plain-JSON` (нетегированный, в отличие от `store/codec.encodeValue`). Импортёр — `engine`; НЕ `eval`, НЕ `internal/value` [FR-007/010].
