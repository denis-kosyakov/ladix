...
- Демонстрационные программы — `examples/*.ladix` в корне репозитория; из пакета `parser` относительный путь
  `../../../examples/<имя>.ladix`
...
- [X] T046 [P] `gofmt -l .` из `src/` — пусто (SC-007); `go vet ./...` — без замечаний
- [X] T047 `go build ./...` и полный `go test ./...` из `src/` — зелёные, лексер 001 не сломан (SC-007)
- [X] T048 Сквозная проверка набора parse-clean (SC-001): `examples/{hello,арифметика,условие,цикл,функция,факториал}.ladix` и `examples/ошибка.ladix` → 0 синтаксических ошибок; декларативные `examples/{выручка,онбординг}.ladix` ожидаемо дают `неожиданный токен` (в набор не входят) — закрепить в `src/internal/parser/examples_test.go`
- [X] T049 [P] Прогон чек-листа приёмки quickstart.md §3 (гейты P1/P2a/P2b/P2c) и сверка с SC-002…SC-006
- [X] T050 [P] Полировка пакет-документации `src/internal/ast/doc.go` и `src/internal/parser/doc.go`: границы (НЕ eval/семантика/декларации), ссылки на contracts; сверить отсутствие импорта `errors` в `ast` (листовость, D1)
