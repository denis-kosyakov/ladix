...
**Performance Goals**: прикладной масштаб; жёстких целей нет. Семпроход процесса — лёгкий
...
- **`выручка.ladix`** в 005 целиком НЕ парсится (содержит `процесс` **и** `когда`) — это **не** баг
  005: процесс теперь парсится, падение сдвигается на `когда` (триггеры — 007, §PM-3). Это демо,
  не golden-кейс приёмки.
