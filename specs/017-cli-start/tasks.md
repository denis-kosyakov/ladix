...
- Фикстура: **ПЕРЕИСПОЛЬЗОВАТЬ существующую** `examples/контроль_плана.ladix` (РЕПО-КОРЕНЬ, НЕ `src/`)
  — она уже содержит `процесс эскалация_плана(факт)`, шаг `связаться_с_клиентом`, `срок: 3дн` и уже в
  `clean[]` (`src/internal/parser/examples_test.go:25`). CLI-golden-фикстуры живут в
  `src/cmd/ladix/testdata/` — туда копия ТОЛЬКО при нужде изоляции golden.
...
- [x] **T001** Сверить базу @a1ad856: switch подкоманд (`main.go` — run/metric/complete/tasks/serve/emit),
...
- [x] **T002** `go test ./...` + `go vet ./...` + `-race` (где применимо) на чистом дереве — зелёные.
...
- [x] **T003** [Тест ДО, P] **(инверсия b)** `TestParseArgLiteralTypes` (`start_literal_test.go`):
...
- [x] **T004** [Тест ДО, P] **(инверсия b)** `TestParseArgLiteralErrors` (`start_literal_test.go`):
  `99999999999999999999`→ошибка `целое вне диапазона типа Целое`; `2026-13-45`→ошибка парса даты. RED.
- [x] **T005** Реализовать `parseArgLiteral(argv string) (value.Value, error)` (`start.go`) — порядок
...
- [x] **T006** **(инверсия b)** Замок диапазона: убрать проверку `strconv.ParseInt` overflow (вернуть
...
- [x] **T007** [Тест ДО] **(инверсия e)** `TestOpenStoreSelectsBackend` (`start_golden_test.go` или
  отдельный): `openStore("")` → MemoryStore + no-op close; `openStore(tmp.db)` → SQLiteStore + Close,
  файл создан. RED (нет хелпера).
- [x] **T008** Реализовать `openStore(dbPath) (store.Store, func() error, error)` (`main.go` или `start.go`)
  — узкий снимок `main.go:235-244` (contract open-store.md). T007 → GREEN.
- [x] **T009** **(инверсия e, регресс)** Прогнать существующие golden `complete`/`tasks`/`emit`
...
- [x] **T010** Каркас `startMain(rest []string, stdout, stderr io.Writer) int` (`start.go`): разбор
...
- [x] **T011** [Тест ДО] **(инверсия a)** `TestStartArityMismatch` (`start_golden_test.go`): процесс
...
- [x] **T012** [Тест ДО] **(инверсия c)** `TestStartUnknownProcess` (`start_golden_test.go`):
  `start f неизвестный 5` → stderr `ladix: процесс 'неизвестный' не объявлен` exit 2 (НЕ текст движка
  `не найден в определении`). RED.
- [x] **T013** Реализовать в `startMain`: `pd, ok := interp.Process(name)`; `!ok` → FR-015 exit 2;
...
- [x] **T014** [Тест ДО→GREEN] `TestStartZeroArity` (`start_golden_test.go`): процесс без параметров,
...
- [x] **T015** [Тест ДО] **(инверсия d)** `TestStartGoldenCanon` (`start_golden_test.go`): фикстура с
...
- [x] **T016** Реализовать в `startMain`: `parseWebhookCaller` → `withExternalCallerOpt` → `NewEngine` →
...
- [x] **T017** [Тест ДО→GREEN] `TestStartTerminalNoTasks` (`start_golden_test.go`): терминальный процесс
  без человеческих задач → stdout = ровно `запущен инстанс p-000001` (движок тих, нет `[задача]`).
- [x] **T018** [Тест ДО] **(инверсия b конец-в-конец)** `TestStartLiteralBinding` (`start_golden_test.go`):
...
- [x] **T019** [Тест ДО] `TestStartBadLiteralCLI` (`start_golden_test.go`): `start f P 99999999999999999999`
...
- [x] **T020** [Тест ДО] **(вебхук)** `TestStartBadWebhookURL` (`start_golden_test.go`):
...
- [x] **T021** [Тест ДО→GREEN] **(инверсия e)** `TestStartDefaultDB` (`start_golden_test.go`): `start f P 1`
...
- [x] **T022** [P] **ПЕРЕИСПОЛЬЗОВАТЬ** `examples/контроль_плана.ladix` (репо-корень) для start-golden —
  фикстура УЖЕ существует (`процесс эскалация_плана(факт)` + шаг `связаться_с_клиентом` + `срок: 3дн`) и
  УЖЕ в `clean[]` (`src/internal/parser/examples_test.go:25`). НЕ создавать дубль, НЕ мутировать `clean[]`
  (счётчик не дрейфит). Если golden требует изолированную фикстуру (напр. `исполнитель: менеджер` дословно
  под канон §AU-10.D, иной дедлайн) — положить копию в `src/cmd/ladix/testdata/`, НЕ в `examples/`.
  Сверить, что первый шаг даёт `[задача] … менеджер … связаться_с_клиентом` под канон.
- [x] **T023** **(регресс, INV-3)** `go test ./...` + `go vet ./...` (+`-race` где есть) — ВЕСЬ v1/M1/M-DX/
...
- [x] **T024** [P] Комментарии-якоря в `start.go`: ссылки §AU-7/§AU-9/§AU-10/§AU-4.5, заметка «арность
  проверяется ДО engine.Start (движок даёт др. текст engine.go:69)», «вебхук в start — паритет complete
  §AU-4.5». `gofmt`.
- [x] **T025** [P] Сверка дословности всех текстов с §AU-10.C/§AU-10.D (4 CLI-ошибки + 2-строчный stdout
  канон) — буква-в-букву. Проверить, что `usage`-строка обновлена и связна.
- [x] **T026** Финальный прогон `go build ./...` (один бинарник, CGO нет) + `go test ./cmd/ladix/...`
