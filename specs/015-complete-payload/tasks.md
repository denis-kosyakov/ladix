...
- [ ] **T001** Сверить базу на `aebac92`: `jsonval.PayloadToRecord` экспортирован
...
- [ ] **T002** `go test ./...` + `go vet ./...` + race на чистом дереве — зелёные
...
- [ ] **T003** [Регресс-замок, ДО правок] Тест `TestCompleteNoPayloadRegress`
  ...
  `--данные` → красный.**
- [ ] **T004** Протянуть `data value.Запись` через 4 функции
...
- [ ] **T005** Обновить ВСЕ существующие вызовы `Complete`/`catchUp`/
...
- [ ] **T006** Проба-инвариант: `ProcessRuntime` остался 8 методов
...
- [ ] **T007** [Замок a, ДО реализации] `TestCompleteFirstStepSeesPayload`
...
- [ ] **T008** [P] `TestPayloadTypesInStep`: payload `{"сумма":2500000}` → шаг видит
  `данные.сумма` как `Целое`; `{"клиент":{"имя":"А"}}` → `данные.клиент.имя` →
  `Строка "А"` (вложенность/числа — семантика jsonval).
- [ ] **T009** [P] `TestPayloadThroughCatchUp`: ветка догона `caughtUp=true`
...
- [ ] **T010** В `advance` (`engine.go`): `cur := data` перед циклом; внутри цикла
...
- [ ] **T011** [Замок b-1, ДО реализации] `TestSecondStepSeesEmpty`
...
- [ ] **T012** [Замок b-2] `TestPayloadNotPersisted`
...
- [ ] **T013** [Замок c, ДО реализации] `TestNoFlagEmptyRecord`
...
- [ ] **T014** [P] `TestPayloadReadOnly`: тело шага читает `данные.поле` (ок), но
  `присвоить данные = …` → ошибка барьера (read-only, как тело триггера). **Инверсия:
  снять барьер → переприсвоение проходит → замок ловит.**

### Реализация

- [ ] **T015** В `advance`: после первой итерации цикла `cur = value.NewRecord(nil,
...
- [ ] **T016** [Замок d, ДО реализации] `TestCompleteBadJSON`
...
- [ ] **T021** [Замок jsonval-дубль] Проба: `complete`-путь зовёт ровно
...
- [ ] **T022** [P] Витрина (если в scope §AU-11): НОВЫЙ `.ladix` + golden демо
  `complete --данные`; детерминизм FixedClock; ошибочную фикстуру НЕ перезаписывать.
- [ ] **T023** [P] `go vet ./...` чисто, `gofmt`, `go test -race ./...` зелёный;
  подтвердить 0 новых зависимостей (`go.mod`/`go.sum` без правок payload-related).
- [ ] **T024** Финальный проход quickstart.md (4 сценария) вручную/скриптом; сверка
