...
**Интервал положителен (SCHED-1).** `каждые <amount><ед>` с `amount ≤ 0` (напр. `каждые 0мин`)
отвергается семпроходом — дословно `интервал расписания должен быть положительным: 'каждые
<amount><ед>'`: иначе `shiftEvery(T, 0, …) == T` → `now >= T` на каждом тике → busy-fire (для тела
«запустить процесс» — лавина инстансов). Проверка единиценезависима (analyze.go §ScheduleTrigger).

**Край v1 — переполнение огромного интервала (SCHED-2, осознанный).** `amount > 0`, но настолько
большой, что `time.Duration(amount)*ед` (фикс-единицы) либо `int(7*amount)`/`int(amount)` (нед/мес)
переполняет int64 → `shiftEvery` даёт `next` в прошлом → busy-fire. Семпроход это НЕ ловит (значение
положительно и в пределах int64). Верхняя граница `amount` в v1 НЕ вводится: интервалы такого
порядка (≈сотни лет+) практического смысла не имеют — задокументировано как осознанный край v1, не
баг. Закрытие — посунитная верхняя граница в будущей версии.
...
var ErrTriggerStateNotFound = errors.New("trigger state not found")
```
