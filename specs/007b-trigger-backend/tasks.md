...
- [ ] T003 [P] Объявить сентинел `ErrTriggerStateNotFound = errors.New("trigger state not found")` в `src/internal/store/types.go` (рядом с сентинелами 006 `ErrInstanceNotFound`/`ErrTaskNotFound`/`ErrTaskAlreadyCompleted` в общем блоке `var ( … )`; явный тип, Принцип III; развёртка `errors.Is`, data-model.md §сентинел)
