...
select { case <-ctx.Done(): return nil; default: d.tick() }  // НЕМЕДЛЕННЫЙ первый тик в t=0 (CONC-3)
for {
    select {
...
- Первый тик — НЕМЕДЛЕННЫЙ в `t=0` (CONC-3): `time.NewTicker` не стреляет в `t=0`, поэтому
  дрена событий / прайминг метрик-edge / якорь расписаний не ждут полного `--interval`; далее —
  периодически по тикеру. Первый тик остаётся ПРАЙМИНГОМ (метрика заводит базу, не фаерит).
  Отмена `ctx` ДО `t=0` (грациозный shutdown до первого тика) → тик не выполняется.
- Рестарт-скан — НЕ внутри `Run`: это отдельный метод `RunRestartScan()`, вызываемый `serveMain`
  ДО `Run(ctx)` (см. serve-command.md §жизненный цикл и §рестарт-скан ниже, FR-019).
