...
> **Восемь пакетов.** Технические рамки перечисляют семь пакетов; этот документ добавляет восьмой листовой пакет `internal/value/` (вариант A, §2.1). Это осознанное доопределение раскладки, а не противоречие: `Value` делят три пакета (`eval`, `store`, `engine`), и без нейтрального места его пришлось бы объявлять в `eval`, втягивая `store`/`engine` в зависимость от интерпретатора (вариант B, отвергнут в §2.1).
...
| `engine` | `internal/engine/` | Движок процессов как библиотека над `Store`: `Start`/`advance`/`Complete`; реализует `eval.ProcessRuntime` (D-1, §2.1); engine-Clock (D-2). Планировщик `serve` (тикер триггеров) — фича 007. Не сервис, без глобалов. |
...
- `engine` → `eval`, `store`, `ast`, `value`, `errors` (движок реализует `eval.ProcessRuntime`, исполняет тела шагов через экспортируемую поверхность `eval`, резолвит `ProcessDecl`/`StepDecl` из `ast`, оперирует `Value`, персистит через `store`).
...
- **Граница run vs serve.** `run` гонит весь пайплайн синхронно от старта до конца программы: без `--db` — на `MemoryStore` (эфемерно), с `--db` — на `SQLiteStore` (мост в персист, Q2 фичи 006). `complete <file.ladix> <task-id>` входит в пайплайн до стадии 4 (компиляция файла обязана пройти чисто, Q3) и дальше работает над `SQLiteStore` и движком; `tasks` файла не принимает — только `SQLiteStore`. `serve`/`emit` — фича 007. `metric`/`repl`, как и `run` без `--db`, остаются на `MemoryStore` (`README, раздел CLI`; канон команд 006 — `docs/engine-model.md §EN-6`).
...
Контракт 006 — **нарезанный** (D-3): ровно 8 методов и 3 сентинела.
...
`ListUnprocessedEvents`/`MarkEventProcessed`) и сентинел `ErrTriggerStateNotFound` в 006
**не объявляются** — фича 007 добавит их аддитивно (эскиз — `docs/execution-model.md` EM-5,
планировщик — §7.5 ниже). **Методов листинга инстансов нет** (D-4: рестарт-скан — носитель
007; сбойное окно `complete` закрывает гард-догон, см. ниже).
...
- **Две реализации:** `MemoryStore` (`run` без `--db`, тесты lifecycle — карты + счётчики id под одним `sync.Mutex`, без JSON) / `SQLiteStore` (`run --db`/`complete`/`tasks` в 006; `serve`/`emit` — 007; WAL + `busy_timeout=5000` + `foreign_keys=ON`).
...
Структуры триггеров/событий — **фича 007** (эскиз, в 006 не объявляются):
...
`engine` — обычный Go-пакет над `Store`, не сетевой сервис и не глобал. Методы `Start`/`advance`/`Complete` переиспользуются и в CLI-командах, и (с 007) в демоне `serve`. «Продвинуть процесс» — синхронный вызов в том процессе ОС, который инициировал событие (CLI-команда или тик демона). Сетевого слоя в v1 нет.
...
Определения процессов движок берёт из интерпретатора (`interp.Process(name)`, `docs/engine-model.md §EN-4`) — для `lookup(ProcessName, CurrentStep)` и «следующего шага по исходнику»; ранний эскиз с картой `procs map[string]*ast.ProcessDecl` в сигнатуре **отменён**. Опции (`Option`): в 006 единственная — `WithClock` (D-2: подмена часов в тестах/golden; дефолт — `SystemClock{}`). Триггеры и интервал тика — фича 007 (добавятся аддитивно, см. §7.5). Engine-Clock используется для `CreatedAt`/`UpdatedAt`/`CompletedAt`, абсолютизации дедлайна (D-19) и `просрочена`; `eval.Clock` (дневной, golden 2026-05-31) не трогается.
...
- На рестарте: инстансы в `ожидает` корректны; застрявшие в `выполняется`/`создан` — переактивируются (`advance` прогоняет шаг заново). **Носитель рестарт-скана — демон `serve` (фича 007)**: в 006 листинга инстансов в `Store` нет (D-3/D-4), залипший в `выполняется` инстанс (kill посреди шага) остаётся до 007; сбойное окно «задача завершена, advance не успел» закрывает гард-догон `complete` (D-4, §6).
...
### 7.5. Планировщик `serve` (`docs/execution-model.md`) — фича 007
...
| `serve <file>` | SQLite | демон-планировщик (`--db`, `--interval`, `--max-depth`) — **фича 007** |
| `emit <событие> [json]` | SQLite | поставить внешнее событие в очередь (`--db`) — **фича 007** |

> Команда `repl` (интерактивный режим) — **вне 006** (бонус, фича не назначена); в этой таблице перечислены Store-режимы пакетного запуска, полный список и семантику команд см. `README, раздел CLI`.

`ladix start` в v1 **отсутствует** — запуск процесса идёт через `запустить процесс` в программе под `run` (мост в персист — `run --db`, Q2) или, с 007, через триггер под `serve`; внешний канал данных — у `emit` (007).
...
- **Каноничный `Store` v006** — `docs/engine-model.md §EN-2` (`SaveInstance`/`LoadInstance`/`MarkTaskCompleted`/…, ровно 8 методов — D-3; §6 синхронизирован с ним); расширение триггерными методами — фича 007 (`docs/execution-model.md` EM-5). Устаревший эскиз `SaveProcess`/`LoadProcess` не воспроизводить.
