...
Три листовых пакета внизу (`errors`, `value`, `jsonval`) плюс почти-листовой `ast` (зависит только от stdlib); `main` — корень, импортирует всё. Рёбра (читать «X → Y» как «X импортирует Y»):
...
```
 ...
 errors             jsonval     jsonval
                    errors
   └──── errors / value / jsonval / ast (листовые) ────┘
```
